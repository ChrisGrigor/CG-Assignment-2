...
void SceneBuilder::Initialize()
{
	...
	//Load Level (Test Time it takes)
	MeshData dataLevel = ObjLoader::LoadObj("A2_Scene1.obj", glm::vec4(1.0f));

	//Load Level (Test Time it takes)
	MeshData dataChar = ObjLoader::LoadObj("Character.obj", glm::vec4(1.0f));
	...
	// Room 
	Material::Sptr roomMat = std::make_shared<Material>(shader);
	roomMat->Set("s_Albedo", Texture2D::LoadFromFile("Level_1UVs.png", false, true, true));
	...
	// Creates our main camera
	{
		...
		buffer->AddAttachment(emissiveBuffer); // Attach the emissive buffer to the G-Buffer
